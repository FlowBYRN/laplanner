...
import { TrainingsAppointmentClient, TrainingsAppointmentDto, TrainingsExerciseClient, TrainingsModuleTrainingsExerciseDto, TrainingsExerciseDto, TrainingsModuleClient, TrainingsModuleDto, TrainingsModuleTagDto, TrainingsModuleTagClient, TrainingsModuleTrainingsModuleTag } from 'src/clients/api.generated.clients';
import { ApplicationUser, UserClient } from 'src/clients/is4.generated.clients';
...
export class ModuleplannerPageComponent implements OnInit {
  ...
  async ngOnInit() {
    this.trainingsModules = await this.trainingsModuleClient.getTrainingsModulesByUserId((await this.getCurrentApplicationUser()).id).toPromise();
  ...
  async addModule() {
    if (this.currentModule != null && this.currentModule.title == "neues Modul") { alert("Bitte ändere zuerst den Namen des aktuellen Moduls!"); }
      ...
      const user = await this.getCurrentApplicationUser();
  ...
  async saveModule() {
    if (this.currentModule.title == "neues Modul") alert("Bitte ändere zuerst den Modulnamen!");
      ...
      await this.userClient.allowEditModule(ret.id, (await this.getCurrentApplicationUser()).id).toPromise();
      console.log(ret);
  ...
  async deleteTrainingsModule(module: TrainingsModuleDto) {
    if (module.id != 0) {
      ...
      console.log(temp);
      await this.userClient.disallowEditModule(module.id, (await this.getCurrentApplicationUser()).id).toPromise();
  ...
  async getCurrentApplicationUser(): Promise<ApplicationUser> {
    return await this.userClient.getUserByName(await this.authService.getCurrentUserName()).toPromise();
  }
}